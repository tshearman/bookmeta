...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="openai",
        help="LLM provider to use for metadata extraction and ranking.",
    )
    parser.add_argument(
        "--invalidate-cache",
        action="store_true",
...
def load_credentials(secrets_path: Path, provider: str) -> tuple[OpenAI | None, str]:
    ...
    client = (
        OpenAI(api_key=openai_api_key, project=openai_project_id)
        if provider == "openai"
        else None
    )
    return client, google_book_api_key
