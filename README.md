...
Modern Python package for extracting, enriching, and embedding metadata in book PDFs. The codebase uses the standard `src/` layout with packaging defined in `pyproject.toml`. We recommend [uv](https://docs.astral.sh/uv/) for dependency management and running the CLI tools.
...
# If the `results-db` is not sufficiently populated,  
# the keyword normalization stage may fail.
...
- `uv run bookmeta-dedupe /path/to/pdfs --dest /tmp/duplicates` – walks the directory tree, hashes every PDF, and moves duplicate files into the destination directory (adding suffixes when names collide). Pass `--rm` instead of `--dest` to delete duplicates in place:

  ```bash
  uv run bookmeta-dedupe ~/Books --rm
  ```

  The tool shares the same hashing logic as the batch pipeline, so you can safely clean noisy directories before invoking `bookmeta-batch`.
